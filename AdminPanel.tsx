  ...
  onChange: (isChecked: boolean) => void;
    ...
    <div className="flex items-center justify-between p-3 bg-light-bg dark:bg-brand-bg rounded-md">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" checked={isChecked} onChange={(e) => onChange(e.target.checked)} className="sr-only peer" />
...
const AdminPanel: React.FC<AdminPanelProps> = ({ movies, setMovies, onLogout, currentUser }) => {
    ...
    const [allComments, setAllComments] = useState<CommentWithMovieInfo[]>([]);
    const [commentSearch, setCommentSearch] = useState('');
    const [searchedUsername, setSearchedUsername] = useState('');
    const [filteredComments, setFilteredComments] = useState<CommentWithMovieInfo[]>([]);
    const [adminReplyingTo, setAdminReplyingTo] = useState<CommentWithMovieInfo | null>(null);
    ...
    const handleAddAdmin = (e: React.FormEvent) => {
        ...
        const result = addAdmin({ username: newAdminUsername.trim(), password: newAdminPassword.trim() });
        if (result.success) {
            setAdmins(getAdmins()); // Re-fetch the list of admins
            setNewAdminUsername('');
            setNewAdminPassword('');
    ...
    const handleRemoveAdmin = (username: string) => {
        if (window.confirm(`Are you sure you want to remove the admin "${username}"? They will no longer be able to log in.`)) {
    ...
    const handleSavePermissions = () => {
        if (editingPermissionsFor && tempPermissions) {
            if (updateAdminPermissions(editingPermissionsFor.username, tempPermissions)) {
                setAdmins(getAdmins()); // Refresh the admin list
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        if (sidebarRef.current) {
            ...
            const minWidth = 400; // Minimum sidebar width in pixels
            const maxWidth = window.innerWidth * 0.98; // Max width to leave a small gap
    ...
    const loadAllComments = () => {
        const commentsFromStorage = getAllCommentsFromStorage();
        const commentsWithTitles = commentsFromStorage.map(c => ({
            ...c,
    ...
    const handleCommentSearch = (e: React.FormEvent) => {
        ...
        const results = allComments.filter(c => c.name.toLowerCase() === commentSearch.trim().toLowerCase());
    ...
    const handleAdminReplySubmit = (e: React.FormEvent) => {
        ...
        try {
            const existingComments: CommentType[] = JSON.parse(localStorage.getItem(storageKey) || '[]');
    ...
    const handleDeleteComment = (commentToDelete: CommentWithMovieInfo) => {
        const commentTextSnippet = commentToDelete.text.length > 50 ? `${commentToDelete.text.substring(0, 50)}...` : commentToDelete.text;
        if (window.confirm(`Are you sure you want to delete this comment by ${commentToDelete.name}?\n\n"${commentTextSnippet}"\n\nThis action will also delete all replies and cannot be undone.`)) {
            if (deleteCommentFromStorage(commentToDelete.movieId, commentToDelete.id)) {
                // Reload all comments from storage to get the fresh state
                const freshCommentsWithTitles = getAllCommentsFromStorage().map(c => ({
                    ...c,
                    movieTitle: movieMap.get(c.movieId) || 'Unknown Movie'
                }));
                setAllComments(freshCommentsWithTitles);

                // If a search is active, re-apply the filter on the fresh data
                if (searchedUsername) {
                    const results = freshCommentsWithTitles.filter(c => c.name.toLowerCase() === searchedUsername.trim().toLowerCase());
                    setFilteredComments(results);
                }
            } else {
                alert("Failed to delete the comment. It might have been already removed.");
            }
        }
    };
    ...
    const handleDownloadLinkChange = (index: number, field: keyof DownloadLink, value: string) => {
        const updatedLinks = formState.downloadLinks ? [...formState.downloadLinks] : [];
        if (!updatedLinks[index]) updatedLinks[index] = { quality: '', url: '' };
        updatedLinks[index][field] = value;
        setFormState(prev => ({ ...prev, downloadLinks: updatedLinks }));
    };
    
    const addDownloadLink = () => {
        setFormState(prev => ({ ...prev, downloadLinks: [...(prev.downloadLinks || []), { quality: '', url: '' }] }));
    };
    
    const removeDownloadLink = (index: number) => {
        setFormState(prev => ({ ...prev, downloadLinks: prev.downloadLinks?.filter((_, i) => i !== index) || [] }));
    };

    const handleTelegramLinkChange = (index: number, field: keyof TelegramLink, value: string) => {
        const updatedLinks = formState.telegramLinks ? [...formState.telegramLinks] : [];
        if (!updatedLinks[index]) updatedLinks[index] = { quality: '', fileId: '' };
        updatedLinks[index][field] = value;
        setFormState(prev => ({ ...prev, telegramLinks: updatedLinks }));
    };

    const addTelegramLink = () => {
        setFormState(prev => ({ ...prev, telegramLinks: [...(prev.telegramLinks || []), { quality: '', fileId: '' }] }));
    };

    const removeTelegramLink = (index: number) => {
        setFormState(prev => ({ ...prev, telegramLinks: prev.telegramLinks?.filter((_, i) => i !== index) || [] }));
    };
    ...
    const addSeason = () => {
        ...
        setFormState(prev => ({ ...prev, seasons: [...(prev.seasons || []), newSeason] }));
    };

    const removeSeason = (seasonIndex: number) => {
        setFormState(prev => ({ ...prev, seasons: prev.seasons?.filter((_, i) => i !== seasonIndex) || [] }));
    };
    
    const addEpisode = (seasonIndex: number) => {
        const newSeasons = [...(formState.seasons || [])];
        const season = newSeasons[seasonIndex];
        const newEpisodeNumber = season.episodes.length + 1;
        const newEpisode: Episode = { episodeNumber: newEpisodeNumber, title: '', downloadLinks: [], telegramLinks: [] };
        season.episodes.push(newEpisode);
        setFormState(prev => ({ ...prev, seasons: newSeasons }));
    };
    ...
    const handleEpisodeChange = (seasonIndex: number, episodeIndex: number, field: keyof Omit<Episode, 'downloadLinks' | 'episodeNumber' | 'telegramLinks'>, value: string) => {
    ...
    const removeEpisodeDownloadLink = (seasonIndex: number, episodeIndex: number, linkIndex: number) => {
        const newSeasons = JSON.parse(JSON.stringify(formState.seasons || []));
        newSeasons[seasonIndex].episodes[episodeIndex].downloadLinks = newSeasons[seasonIndex].episodes[episodeIndex].downloadLinks.filter((_: any, i: number) => i !== linkIndex);
        setFormState(prev => ({ ...prev, seasons: newSeasons }));
    };
    ...
    const removeEpisodeTelegramLink = (seasonIndex: number, episodeIndex: number, linkIndex: number) => {
        const newSeasons = JSON.parse(JSON.stringify(formState.seasons || []));
        newSeasons[seasonIndex].episodes[episodeIndex].telegramLinks = newSeasons[seasonIndex].episodes[episodeIndex].telegramLinks.filter((_: any, i: number) => i !== linkIndex);
        setFormState(prev => ({ ...prev, seasons: newSeasons }));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const movieData = {
          ...
          downloadLinks: formState.category !== 'webseries' ? formState.downloadLinks?.filter(link => link.quality && link.url) || [] : [],
          telegramLinks: formState.category !== 'webseries' ? formState.telegramLinks?.filter(link => link.quality && link.fileId) || [] : [],
          seasons: formState.category === 'webseries' ? formState.seasons?.filter(s => s.episodes.length > 0) || [] : [],
        };
        ...
        if (isEditing && editingMovieId) {
            ...
            const updatedMovie: Movie = { id: editingMovieId, ...movieData };
            setMovies(prev => prev.map(m => m.id === editingMovieId ? updatedMovie : m));
        } else {
            if (!hasPermission('canAddContent')) {
                alert("You don't have permission to add content.");
                return;
            }
            const newMovie: Movie = { id: new Date().toISOString(), ...movieData };
    ...
    const handleRemove = (id: string, title: string) => {
        ...
        if (window.confirm(`Are you sure you want to remove "${title}"? This action cannot be undone.`)) {
            setMovies(prev => prev.filter(m => m.id !== id));
            if(id === editingMovieId) {
                resetForm();
            }
    ...
    const handleTmdbSearch = async () => {
        ...
        try {
            const response = await fetch(`${TMDB_BASE_URL}/search/multi?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(tmdbSearchQuery)}`);
            if (!response.ok) throw new Error('Failed to fetch from TMDB.');
            const data = await response.json();
            const validResults = data.results.filter((r: any) => (r.media_type === 'movie' || r.media_type === 'tv') && r.poster_path);
            setTmdbResults(validResults);
            if (validResults.length === 0) {
              setTmdbError('No results found.');
            }
        } catch (error) {
            setTmdbError(error instanceof Error ? error.message : 'An unknown error occurred.');
        } finally {
    ...
    const handleSelectTmdbResult = async (result: TmdbSearchResult) => {
        ...
        try {
            const detailsRes = await fetch(`${TMDB_BASE_URL}/${result.media_type}/${result.id}?api_key=${TMDB_API_KEY}&append_to_response=credits,videos`);
    
            if (!detailsRes.ok) throw new Error('Failed to fetch details from TMDB.');
    
            const details: TmdbDetailResponse = await detailsRes.json();
            const credits = details.credits;
            
            const director = credits.crew.find(c => c.job === 'Director')?.name || details.created_by?.[0]?.name || 'N/A';
            const producers = credits.crew.filter(c => c.job === 'Producer').map(p => p.name).slice(0, 3).join(', ') || 'N/A';
            const actors = credits.cast.slice(0, 5).map(a => a.name).join(', ') || 'N/A';
            
            const officialTrailer = details.videos.results.find(v => v.type === 'Trailer' && v.site === 'YouTube' && v.official) || details.videos.results.find(v => v.type === 'Trailer' && v.site === 'YouTube');

            setFormState(prev => ({
                ...prev,
                title: details.title || details.name || '',
                posterUrl: details.poster_path ? `${TMDB_IMAGE_BASE_URL}w500${details.poster_path}` : '',
                backdropUrl: details.backdrop_path ? `${TMDB_IMAGE_BASE_URL}w1280${details.backdrop_path}` : '',
                description: details.overview || '',
                actors: actors,
                director: director,
                producer: producers,
                rating: parseFloat(details.vote_average.toFixed(1)) || 0,
                category: result.media_type === 'tv' ? 'webseries' : prev.category,
                genres: details.genres || [],
                releaseDate: details.release_date || details.first_air_date || '',
                runtime: details.runtime || (details.episode_run_time && details.episode_run_time[0]) || 0,
                tagline: details.tagline || '',
                trailerLink: officialTrailer ? getYoutubeEmbedUrl(officialTrailer.key) : '',
                telegramLinks: [],
            }));
            ...
            setTmdbError(error instanceof Error ? error.message : 'An unknown error occurred while fetching details.');
        } finally {
            setIsTmdbLoading(false);
        }
    };
                        ...
                        {commentsToDisplay.length > 0 ? commentsToDisplay.map(comment => (
                            <div key={comment.id} className="p-3 bg-light-card dark:bg-brand-card rounded-lg shadow-sm animate-fade-in">
                                <div className="flex justify-between items-center text-xs">
                                    <p className="font-bold text-light-text dark:text-brand-text">{comment.name}</p>
                                    <p className="text-light-text-secondary dark:text-brand-text-secondary">{new Date(comment.timestamp).toLocaleString()}</p>
                                </div>
